...
LXHardwareSerial LXSerial2(1);    //must be initialized before ESP32DMX
LX32DMX ESP32DMX;
...
static void rdmTask(void *param) {
    ...
    while (ESP32DMX.continueTask()) {
        ...
        if (c >= 0) {                                // if byte read, receive it...may invoke callback function
            ESP32DMX.byteReceived(
                    c & 0xff);            // eventually will catch up, nothing to read and vTaskDelay will be called
        } else {
            ...
            if (task_mode) {
                if (task_mode == DMX_TASK_SEND_RDM) {
                    ...
                    LXSerial2.write(ESP32DMX.rdmData(),
                                    ESP32DMX.rdmPacketLength());        // data should be set from function that sets flag to get here
                    ...
                    LXSerial2.sendBreak(
                            150);                    // send break first (uses hardwareSerialDelayMicroseconds)
                    ...
                    //vTaskDelay must be called to avoid wdt and lock up issues
                    vTaskDelay(
                            5);                            //use time while UART finishes sending to allow other tasks to run
                    LXSerial2.waitFIFOEmpty();                //returns at about byte 384 ~128 bytes left 128 * 44 = 5.6 ms
                    LXSerial2.waitTXDone();
    ...
    if (_receivedData[0] == 0) {                //zero start code is DMX
        if (_rdm_read_handled == 0) {
            if (_current_slot > 24) {
                ...
                xSemaphoreTake(ESP32DMX.lxDataLock,
                               portMAX_DELAY);    // double buffer makes task conflict unlikely, but use mutex anyway
